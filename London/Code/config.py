...
params_dict['batch_size'] = [1,1,1]               #batch size during training, validation, and prediction
params_dict['learning_rate'] = 0.001                  #initial learning rate 
params_dict['weight_decay'] = 0.00002               #initial weight decay (only used for optimizers that accept weight decay parameter -> AdamW and SGDW)
...
params_dict['adaptive_full_image_patching'] = [False, True]             #whether to train on full size images, and whether to validate on full size images (in order to get accurate validation set dice score)
...
params_dict['crop_input_image_for_augmentation'] = [True, 10, False]    #whether to crop the input images when augmenting the images (will speed up augmentation, but may cause artifacts in patches); amount of voxels to keep around patch to limit artifacts; whether to account for voxel spacing and use the downsample_for_augmentation parameter (suggested to be False)
params_dict['probability_augmentation'] = [0.3,.3,0,.3,.3,.3,.3]                            #probability of applying each of the requested transforms (if supplying list of probabilities, must be of length 7 (one probability for flip, gamma, deform, scale, rotate, shear, translate))
...
params_dict['deform_mesh'] = [False, [15.0, 50.0], 3.5, 3]                                   #whether to apply deformation augmentation, scale factors for normal/abnormal, sigma of gaussian smoother, truncate filter sigma, spline order for interpolation of volume and label 
params_dict['scale_mesh'] = [True, [.70, 1.4], False]                                      #whether to apply scaling augmentation, and the bounds for scaling operation (less than 1 zooms in; greater than 1 zooms out); whether scaling should be isotropic or not
...
params_dict['shear_mesh'] = [True, [-.20, .20], False]                                      #whether to apply shear augmentation, and the bounds for shear operation (values close to zero will produce sensible shears); whether to shear only along one direction, or to allow shearing along all valid axes
params_dict['translate_mesh'] = [True, [-5, 5]]                                             #whether to apply translation augmentation, and the bounds for translating operation (in voxels)
...
params_dict['spline_order'] = [1, 0, 1]                         #spline order for applying deformations to volumes and label maps (and distance maps if applicable)
params_dict['num_epochs'] = 500                                #number of epochs to train the model
params_dict['early_stopping_threshold'] = [100, 30, 5.]        #number of epochs before breaking from training loop if no improvement in validation monitor, number of epochs to drop learning rate (if not using LR schedule), factor by which to drop learning rate
...
params_dict['workers'] = psutil.cpu_count(logical=False)-1                                     #number of parallel processes to generate to handle data loading
...
params_dict['num_epochs_cycle'] = 70                                   #number of epochs in the first cycle
params_dict['factor_decrease_learning_rate'] = 350                     #amount to decrease learning rate by the end of the cycle
...
#unet params
params_dict['network_name'] = 'Unet'                                                    #name of network to use during training (choose between 'Unet' and 'ClassificationNetwork'); 'ClassificationNetwork' can be used as a generic ResNet/DenseNet by modulating the options below
...
params_dict['levels'] = 5                                                               #number of levels in u-net
...
params_dict['filter_num_per_level'] = [32,64,128,256,512]                               #number of filters in each layer of the network (must be list of length equal to levels parameter and each successive value must be double the previous or you might run into compilation errors on some of the more complex unets)
params_dict['blocks_per_level'] = [2,1,1,1,1,2,2,2,2]                                                     #number of blocks (i.e. conv->norm->act) per level; if an integer, uses that number of blocks on each level throughout the network, else input a list of length 2*levels-1 with number of blocks on each level (useful if want to create asymmetrical encoder-decoder)
params_dict['dense_growth_rate_per_level'] = 32#[16,16,32,32,32,32,32,16,16]               #growth rate if using dense block; if an integer, uses that same growth rate on each level throughout the network, else input a list of length 2*levels-1 with growth rate of each level
...
params_dict['use_attentive_normalization'] = [False, 8, 32, [10,10,15,15,20,20]]           #whether to use attentive normalization, reduction ratio for bottleneck, minimum size of bottleneck, value of K at each layer of the network
params_dict['loss'] = 'joint_dice_cross_entropy_loss'                                        #loss function (from loss_functions.py file)
params_dict['metrics'] = ['dice_coef_metric', 'hard_dice_coef_metric']                  #metrics (from loss_functions.py file)
params_dict['monitor'] = ['hard_dice_coef_metric', 'max']                               #monitor for saving model, and direction ('min' or 'max') that indicates better model performance 
...
params_dict['dropblock_params'] = None #[None,None,None,[.9,8,[16]*3],[.9,8,[8]*3]]     #probability for dropblock at each level of network and block size at that level
...
params_dict['retina_net_bias_initializer'] = [True, 0.01]                               #whether to use a retina-net style initializer for the bias of the final convolutional layer, and the a priori probability of the foreground class (will help limit massive gradient that occurs due to incorrectly classified background examples)
...
params_dict['factor_reweight_foreground_classes'] = [1., 3.]          #list of size equal to the number of classes (including background class) (i.e. list of size two for binary task, list of size 4 for task with 3 foreground classes and 1 background, etc) with factor to change weighting of each class in cross entropy losses
